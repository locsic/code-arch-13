...
public class QueryHandler {
	...
	public static void ReadUserQuery (String input_query) throws IOException, RecognitionException
	{
		searchNodeTypeArray = new String[10];
		String file = Controller.ROOT + input_query;
	...
	public static void QueryBuilder(CommonTree t, int indent)
	{
		...
		for ( int i = 0; i < t.getChildCount(); i++ ) {
			...
			else if ((t.getChild(i).getText().toString().equals("PRINT_PP")))
			{
				...
				queries.add(query);
				//Flush system by setting flag in main
			...
			for ( int i = 0; i < t.getChildCount(); i++ ) {
				...
				else if (t.getChild(i).getText().toString().equals("contains"))
				{
					...
					else if (t.getChild(i).getText().toString().equals("PROPERTY"))
					{
						...
						GetSearchNode((CommonTree)t.getChild(i), ident+1, query);
					}
